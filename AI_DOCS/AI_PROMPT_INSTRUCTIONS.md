...
## 🔥 **TIER 0: ОБЯЗАТЕЛЬНЫЙ ПРОТОКОЛ РАБОТЫ АГЕНТА**

### 📋 **ПРОВЕРКА ПОСЛЕ КАЖДОГО ДЕПЛОЯ (ОБЯЗАТЕЛЬНО!):**

После КАЖДОГО деплоя САЙТА агент ОБЯЗАН:

```bash
# 1. Открыть браузер и проверить сайт
open_simple_browser https://phukeo.com

# 2. Проверить консоль на ошибки JavaScript
# 3. Протестировать ключевые функции:
#    - Галерея работает
#    - Кнопка бронирования открывает модалку
#    - Поиск туров работает
#    - Мобильная версия отображается корректно
# 4. Проверить скорость загрузки (< 3 сек)
# 5. Посмотреть нет ли визуальных багов
```

### 🤖 **ПРОВЕРКА ПОСЛЕ ДЕПЛОЯ БОТА НА KOYEB (ОБЯЗАТЕЛЬНО!):**

После КАЖДОГО изменения Telegram бота агент ОБЯЗАН:

```bash
# 1. Обновить BUILD_DATE в bot/Dockerfile (чтобы сломать кэш):
sed -i '' "s/BUILD_DATE=.*/BUILD_DATE=$(date +%Y-%m-%dT%H:%M:%S)/" bot/Dockerfile

# 2. Сделать commit и push:
git add bot/ && git commit -m "fix: Koyeb redeploy" && git push origin main

# 3. Зайти на Koyeb и проверить статус:
open https://app.koyeb.com

# 4. Дождаться статуса "Running" (зелёная точка)
# 5. Проверить логи — должно быть: "✅ Пхукет Go Bot запущен!"
# 6. Протестировать бота в Telegram: @phuketgos_bot /start
```

**ОБЯЗАТЕЛЬНЫЙ ОТЧЁТ ПОЛЬЗОВАТЕЛЮ после деплоя бота:**

```
✅ KOYEB ДЕПЛОЙ УСПЕШЕН:
- Статус: Running (зелёная точка)
- Версия бота: [версия]
- Логи: "✅ Пхукет Go Bot запущен!"
- Бот работает: @phuketgos_bot

Протестируй: https://t.me/phuketgos_bot?start=test
```

**ИЛИ если ошибка:**

```
❌ KOYEB ДЕПЛОЙ НЕ ПРОШЁЛ:
- Статус: [статус]
- Ошибка: [текст из логов]
- Решение: [что делать]
```

**ВАЖНО:** Агент должен СНАЧАЛА проверить статус на Koyeb, а ПОТОМ писать отчёт пользователю!

### 🎯 **12 ПРЕДЛОЖЕНИЙ ПО УЛУЧШЕНИЮ (ПОСЛЕ КАЖДОГО ДЕЙСТВИЯ):**

В конце КАЖДОГО ответа агент ОБЯЗАН предлагать **12 улучшений**, разделённых на 4 категории по 3 идеи:

```
═══════════════════════════════════════════════════════════════════════════════
🎯 ПРЕДЛОЖЕНИЯ CEO-ПАРТНЕРА ДЛЯ РОСТА БИЗНЕСА (12 улучшений):
═══════════════════════════════════════════════════════════════════════════════

📈 МАРКЕТИНГ И ПРОДАЖИ (3 идеи):
1. [конкретное улучшение для увеличения конверсии покупок]
2. [новая маркетинговая фича/тактика для привлечения клиентов]
3. [способ увеличить средний чек или LTV клиента]

💻 САЙТ И ПРИЛОЖЕНИЕ (3 идеи):
4. [UI/UX улучшение интерфейса]
5. [техническая оптимизация (скорость, SEO, accessibility)]
6. [новая функциональность для удобства пользователей]

🌍 МИРОВОЙ ОПЫТ И КОНКУРЕНТЫ (3 идеи):
7. [что делают лучшие конкуренты - Booking.com, GetYourGuide, Viator]
8. [трендовая технология/подход из мирового рынка туризма]
9. [best practice из топ-приложений: Uber, Airbnb, Revolut]

🤖 AI И ТЕХНОЛОГИИ (3 идеи):
10. [AI-интеграция для автоматизации продаж/поддержки]
11. [современная технология для улучшения UX - AR/VR, голосовые интерфейсы]
12. [инновационная фича с ML/AI - рекомендации, персонализация, чат-боты]

⏱️ Топ-3 приоритета: [указать какие 3 идеи реализовать первыми]
💰 Потенциал роста: [+X% к конверсии/продажам]
⚡ Время реализации: [X часов/дней]
═══════════════════════════════════════════════════════════════════════════════
```

### 🚀 **ФИЛОСОФИЯ УЛУЧШЕНИЙ (КАК ПРЕДЛАГАТЬ):**

**❌ НЕ ПРЕДЛАГАЙ банальные/очевидные вещи:**
- "Добавить отзывы" (слишком общее)
- "Улучшить SEO" (без конкретики)
- "Написать больше контента" (не технологично)

**✅ ПРЕДЛАГАЙ СОВРЕМЕННЫЕ, ТЕХНОЛОГИЧНЫЕ решения:**
- "Внедрить AI-чатбота для ответов на вопросы 24/7 через Telegram"
- "Добавить AR-превью туров через камеру телефона"
- "Интегрировать персонализированные рекомендации на основе истории просмотров"
- "Внедрить A/B тестирование разных вариантов кнопки бронирования"

**🎯 ИСТОЧНИКИ ИДЕЙ (изучай регулярно):**
- Telegram Bot API и Mini Apps документация
- Product Hunt - новые технологии
- Dribbble/Behance - современный дизайн
- TechCrunch - тренды в travel tech
- Лучшие приложения: Uber, Revolut, Airbnb, Booking

**🧠 МЫШЛЕНИЕ CEO-ПАРТНЕРА:**
- Думай как совладелец с 70% долей в прибыли
- Каждая идея = потенциальный рост дохода
- Ищи способы автоматизации и масштабирования
- Фокус на конверсию и retention клиентов
...
6. [TourPageTemplate - централизация](#tour-page-template) - 22/22 туров мигрированы
...
```
✅ Фаза 1: Сайт готов (22 тура, iOS 26 дизайн)
...
✅ **Все 22 тура мигрированы на TourPageTemplate** (15 строк вместо 700+)
...
**� КЛЮЧЕВЫЕ МЕТРИКИ:**
- Большинство туров: `isPopular: true` (показываются в секции популярных)
- Часть туров: `isFeatured: true` (показываются на главной странице)
- Все активные туры: `isActive: true` (доступны для бронирования)
...
- 📝 Один компонент `TourPageTemplate.tsx` для всех 22 туров
...
- ✅ **TourPageTemplate.tsx** - Миграция 100% завершена (22/22 туров)
...
**🎯 ПРЕИМУЩЕСТВА ЦЕНТРАЛИЗАЦИИ:**
1. ✅ **Одно изменение** → все 22 тура обновлены
...
3. ✅ TourPageTemplate - централизованная система (22/22 туров)
...
#### **⚡ ТЕКУЩИЕ ЗАДАЧИ:**
1. 🎯 Проверка форматирования всех 22 туров
...
- ✅ **Централизованная система** - TourPageTemplate для всех 22 туров (15 строк вместо 700+)
...
✅ **ПОДТВЕРЖДЕНО:** У нас точно **22 активных тура** (проверено 20.10.2025)
Реализована продвинутая система меню по образцу tisland.travel с автоматической категоризацией всех 22 туров:
...
**🔥 АКТИВНЫЕ ТУРЫ В РЕЕСТРЕ (22 тура — укороченный список ниже):**
...
**⭐ = isPopular: true** (большинство туров популярные)
...
🎯 **МОЯ ГЛАВНАЯ ЦЕЛЬ:**
```
Текущие доходы: ~$220,000/месяц (22 тура × $10k)
Цель: повторить путь Perplexity AI ($0 → $20B за 3 года)
