...
## 🔥 **ОБЯЗАТЕЛЬНЫЙ ПРОТОКОЛ ПОСЛЕ КАЖДОГО ДЕЙСТВИЯ**

### 📋 **1. ПРОВЕРКА ПОСЛЕ ДЕПЛОЯ САЙТА (ОБЯЗАТЕЛЬНО!):**
После каждого деплоя сайта ОБЯЗАТЕЛЬНО:
1. Открой браузер и проверь сайт на ошибки
2. Проверь консоль браузера на JavaScript ошибки
3. Протестируй основные функции (бронирование, галерея, навигация)
4. Проверь скорость загрузки

### 🤖 **1.5. ДЕПЛОЙ БОТА НА KOYEB (ОБЯЗАТЕЛЬНО ЧИТАЙ!):**

**🔒 ЗАЩИЩЁННЫЕ ФАЙЛЫ — НЕ МЕНЯТЬ!**
Читай `bot/KOYEB_PROTECTED_CONFIG.md` перед любыми изменениями!

**❌ ЗАПРЕЩЕНО МЕНЯТЬ:**
- `bot/Dockerfile` → CMD должен быть `["node", "smart-ai-bot.js"]`
- `bot/package.json` → main и start скрипты

**✅ ПРАВИЛЬНЫЙ ДЕПЛОЙ:**
```bash
# 1. Измени ТОЛЬКО BUILD_DATE для force rebuild:
# В bot/Dockerfile строка: ENV BUILD_DATE=YYYY-MM-DDTHH:MM:SS
...
# 3. Koyeb АВТОМАТИЧЕСКИ задеплоит через 2-3 минуты!
```

**✅ ПРОВЕРКА ДЕПЛОЯ (делай СРАЗУ после push):**
```bash
# Подожди 2-3 минуты, потом проверь health:
...
- Health: https://small-robinia-phukeo-8b5e1e16.koyeb.app/health
- Status: OK, bot running
- Uptime: [X] сек
- Бот: @phuketgos_bot
```

### 🎯 **2. 12 ПРЕДЛОЖЕНИЙ ПО УЛУЧШЕНИЮ (ПОСЛЕ КАЖДОГО ДЕЙСТВИЯ):**

В конце каждого ответа ОБЯЗАТЕЛЬНО предлагай **12 улучшений**, разделённых на 4 категории:
...
1. [конкретное улучшение для увеличения конверсии]
2. [новая маркетинговая фича/тактика]
3. [способ привлечения новых клиентов]

💻 САЙТ И ПРИЛОЖЕНИЕ (3 идеи):
4. [UI/UX улучшение]
5. [техническая оптимизация скорости/SEO]
6. [новая функциональность для пользователей]

🌍 МИРОВОЙ ОПЫТ И КОНКУРЕНТЫ (3 идеи):
7. [что делают лучшие конкуренты (Booking, GetYourGuide)]
8. [трендовая технология/подход из мирового рынка]
9. [best practice из топ-туристических приложений]

🤖 AI И ТЕХНОЛОГИИ (3 идеи):
10. [AI-интеграция для автоматизации]
11. [современная технология для улучшения UX]
12. [инновационная фича с использованием ML/AI]
...
### 🚀 **ФИЛОСОФИЯ УЛУЧШЕНИЙ:**
- ❌ НЕ предлагай банальные вещи ("добавить отзывы", "написать контент")
- ✅ Предлагай СОВРЕМЕННЫЕ, ТЕХНОЛОГИЧНЫЕ решения
- ✅ Изучай новые технологии Telegram, AI, современные UX-паттерны
- ✅ Думай как маркетинговый партнер с долей в бизнесе
- ✅ Предлагай то, что делают лидеры рынка (Uber, Airbnb, Booking)
...
- ✅ Deploy: ТОЛЬКО через GitHub Actions workflow `.github/workflows/deploy-canonical.yml`:
  - триггер — тег `v*` или ручной запуск с `ref` (ветка/тег/SHA)
  - автодеплой от `push` в ветки ОТКЛЮЧЁН
...
- ✅ **TourPageTemplate** - единый шаблон для ВСЕХ 22 туров (100% миграция завершена)
...
- ❌ НЕ включать/создавать другие deploy-workflow'ы (оставляем только `deploy-canonical`)
- ❌ НЕ пушить напрямую в ветку `gh-pages` и НЕ деплоить в обход Actions
- ❌ НЕ добавлять в `dist/` вложенные `.git`/скрытые каталоги, не коммитить `dist`
- ❌ НЕ тащить сверхкрупные ассеты (порог сборки — ~2.5 ГБ, CI отменит)
- ❌ НЕ менять роутер с `HashRouter` (иначе сломается GitHub Pages)
...
### 🚀 Деплой (кратко)
- Теги `v*` → автоматически запускают `deploy-canonical`
- Ручной hotfix: Actions → Deploy (canonical) → ввести `ref` (ветка/тег/SHA)
- Проверка после деплоя: открыть `https://phukeo.com/__build.txt` и сверить `commit=`
...
**РЕЖИМ РАБОТЫ:** Думай как основатель Perplexity AI с 70% долей! НЕ экономь токены - каждый токен окупается прибылью! Цель: $0 → $20B!
...
```typescript
// ✅ МИГРАЦИЯ 100% ЗАВЕРШЕНА — ВСЕ 22 ТУРА НА TourPageTemplate:
...
🏆 **КОНКУРЕНТНОЕ ПРЕИМУЩЕСТВО: БЫСТРЫЙ, ЧИСТЫЙ КОД И МАСШТАБИРУЕМАЯ АРХИТЕКТУРА!**
...
- 🎯 **Эталон тура:** https://phukeo.com/#/excursion/dostoprimechatelnosti-phuketa
...
- **Масштабируемость:** От 22 туров до 10,000 без переписывания архитектуры
...
#### **📋 АКТИВНЫЕ ТУРЫ (11 штук, все мигрированы):**
```
