...
Этот файл контролирует отображение **ВСЕХ 22 ТУРОВ**. Если он сломается - сломаются ВСЕ туры одновременно!
...
**Размер:** ~800 строк
**Зависимости:** 22 страницы туров зависят от этого файла
...
- Статус: ✅ Работает на всех 22 турах
...
if git diff --cached --name-only | grep -q "src/components/TourPageTemplate.tsx"; then
  ...
  echo "⚠️  ВНИМАНИЕ: Изменения в TourPageTemplate влияют на ВСЕ 22 тура!"
