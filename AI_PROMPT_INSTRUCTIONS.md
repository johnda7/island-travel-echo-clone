...
### 🎯 ГЛАВНОЕ ПРАВИЛО: "ИЗМЕНЕНИЯ В ОДНОМ МЕСТЕ → ЭФФЕКТ ВЕЗДЕ"

#### 🧮 ЦЕНТРАЛИЗОВАННЫЙ КАЛЬКУЛЯТОР:
- **ВСЕГДА используйте `UniversalBookingModal`** для бронирования
- **НИКОГДА не создавайте локальные формы** с useState для adults/children/formData
- **Следуйте WordPress-принципу:** изменения в одном компоненте влияют на все туры

#### 🏷️ ЦЕНТРАЛИЗОВАННЫЕ ТЕГИ:
- **Теги должны быть data-driven** из массива `excursion.tags`
- **НИКОГДА не хардкодите теги** типа `<span>Пхи-Пхи</span>`
- **Используйте `TourTags` компонент** - теги автоматически становятся кликабельными
- **Принцип:** добавил тег в данные → стал кликабельным везде

#### ✅ ПРЕИМУЩЕСТВА ЦЕНТРАЛИЗАЦИИ:
1. **Масштабируемость** - новый тур работает сразу
2. **Консистентность** - единый UX во всех турах  
3. **Легкость поддержки** - исправления в одном месте
4. **Автоматизация** - теги, фильтры, поиск работают сами

#### ❌ ЗАПРЕЩЕННЫЕ ПРАКТИКИ:
```typescript
// ❌ НЕ СОЗДАВАТЬ локальные калькуляторы:
const [adults, setAdults] = useState(1);
const handleBooking = async () => {...};

// ❌ НЕ ХАРДКОДИТЬ теги:
<span className="tag">Пхи-Пхи</span>
<div>Морские экскурсии</div>

// ✅ ПРАВИЛЬНО - централизованно:
<UniversalBookingModal tourData={excursion} />
...
## 🎯 ЦЕНТРАЛИЗОВАННАЯ СИСТЕМА ТЕГОВ И КАЛЬКУЛЯТОРА (2025)

### ✅ СТАТУС: ПОЛНОСТЬЮ РЕАЛИЗОВАНО
**Дата обновления:** 21 сентября 2025

### 🏷️ КЛИКАБЕЛЬНЫЕ ТЕГИ ВО ВСЕХ ТУРАХ:

#### 1️⃣ UNIFIED TAG SYSTEM:
- **`TourTags.tsx`** - централизованный компонент для всех туров
- **Data-driven** - теги берутся из данных тура, НЕ хардкод
- **Кликабельные** - автоматический переход к фильтрации в `/tours?tag=X`
- **Консистентный дизайн** - единый стиль во всех турах

#### 2️⃣ ИНТЕГРАЦИЯ В ТУРЫ:
```typescript
// ✅ ПРАВИЛЬНО - data-driven теги:
<TourTags tags={excursion.tags} />

// ❌ НЕПРАВИЛЬНО - хардкод:
<span>Пхи-Пхи</span>
<span>Морские экскурсии</span>
```

#### 3️⃣ WORDPRESS-ПРИНЦИП "ДОБАВИЛ - ПОЯВИЛОСЬ ВЕЗДЕ":
- Добавил тег в данные тура → автоматически стал кликабельным
- Тег появился в фильтрации `/tours` → пользователи могут найти похожие туры
- Единый источник правды → изменения в одном месте влияют на все

### 🧮 ЦЕНТРАЛИЗОВАННЫЙ КАЛЬКУЛЯТОР:

#### 1️⃣ UNIVERSAL BOOKING MODAL:
- **Единая форма** - `UniversalBookingModal` для ВСЕХ туров
- **Автоматический калькулятор** - принимает любые цены (`priceAdult`, `priceChild`)
- **Удалены локальные формы** - нет дублированного кода

#### 2️⃣ ИСПРАВЛЕННЫЕ ТУРЫ:
- ✅ **PhiPhi2Days1Night.tsx** - использует `UniversalBookingModal`
- ✅ **DostoprimechatelnostiPhuketa.tsx** - использует `UniversalBookingModal`
- ✅ **RassvetnoePrikljuchenie.tsx** - использует `UniversalBookingModal`  
- ✅ **PearlsAndamanSea.tsx** - использует `UniversalBookingModal`

#### 3️⃣ УДАЛЕН ДУБЛИРОВАННЫЙ КОД:
```typescript
// ❌ БЫЛО - локальные состояния в каждом туре:
const [adults, setAdults] = useState(1);
const [children, setChildren] = useState(0);
const [formData, setFormData] = useState({...});
const handleBooking = async () => {...};

// ✅ СТАЛО - централизованно в UniversalBookingModal:
...
### 🔄 ПРАВИЛА ДЛЯ НОВЫХ ТУРОВ:

#### ✅ ОБЯЗАТЕЛЬНО ИСПОЛЬЗОВАТЬ:
1. **UniversalBookingModal** - ЕДИНСТВЕННАЯ форма бронирования
2. **TourTags component** - НЕ создавать теги вручную
3. **Data-driven подход** - теги из `excursion.tags`, НЕ хардкод

#### ❌ ЗАПРЕЩЕНО:
1. **Локальные формы бронирования** - создавать useState для adults/children/formData
2. **Хардкод тегов** - `<span>Пхи-Пхи</span>` вместо `<TourTags>`
3. **Дублирование калькулятора** - каждый тур должен использовать UniversalBookingModal

#### 📋 ЧЕКЛИСТ ДЛЯ НОВЫХ ТУРОВ:
```typescript
// 1. Импортировать компоненты:
import { UniversalBookingModal } from "@/components/UniversalBookingModal";
import TourTags from "@/components/TourTags";

// 2. Добавить теги в данные тура:
const excursion = {
  // ... другие поля
  tags: ["море", "острова", "снорклинг", "пляжи"]
};

// 3. Использовать в JSX:
<TourTags tags={excursion.tags} />
<UniversalBookingModal 
  isOpen={showBookingModal}
  onClose={() => setShowBookingModal(false)}
