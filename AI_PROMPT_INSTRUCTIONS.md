...
// 📋 15 КРИТЕРИЕВ ПРОВЕРКИ - все туры должны соответствовать эталону:

1. 📱 Telegram Web App интеграция (TelegramNav, conditional rendering)
2. 🍞 Breadcrumbs навигация (только в браузере)
3. 🖼️ Галерея фотографий (mobile carousel + desktop grid)
4. 🏷️ Теги категорий (horizontal scroll pills)
5. 📋 Заголовки и метаинформация (H1, subtitle, metrics)
6. 📝 Описание тура (structured prose)
7. ⏰ Программа тура (красивые блоки с временем вместо таблиц)
8. ✨ Особенности тура (bullet points с зелеными маркерами)
9. 💰 Включено/Дополнительные расходы (two-column layout)
10. 🎒 Взять с собой (blue bullets)
11. ⚠️ Важно знать (warning emojis)
12. 🖥️ Desktop sidebar бронирования (sticky card с двумя кнопками)
13. 📱 Mobile booking bar (fixed bottom)
14. 📋 Модальное окно бронирования (UniversalBookingModal)
15. 📊 Структура данных (полная типизация TourData)
...
```typescript
// 🎯 ПЛАН: Проверить каждый тур по 15 критериям эталона
✅ DostoprimechatelnostiPhuketa.tsx (ЭТАЛОН - 100% готов)
✅ PhiPhi2Days1Night.tsx (приведен к стандарту)  
⏳ PearlsAndamanSea.tsx (следующий на проверку)
⏳ JamesBondIsland.tsx
⏳ AvatarPlusHangdong.tsx
⏳ RailaySunset.tsx
⏳ RaftingElephants.tsx
⏳ RachaYaiIsland.tsx
⏳ HongIslandKrabi.tsx
⏳ KaoLakElephants.tsx

// 📋 ПРОЦЕДУРА: Для каждого тура проверить:
- Соответствие всем 15 критериям эталона
- Структура описания (блоки вместо таблиц)
- Кнопки Telegram с точным текстом
- Desktop sidebar с обеими кнопками
