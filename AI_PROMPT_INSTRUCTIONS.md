...
- ✅ **Копируйте структуру из PhiPhi** но заменяйте калькулятор на UniversalBookingModal
...
### 🔧 **РЕКОМЕНДУЕМЫЙ КАЛЬКУЛЯТОР ДЛЯ НОВЫХ ТУРОВ: UniversalBookingModal**

**📌 ДЛЯ ВСЕХ НОВЫХ ТУРОВ ИСПОЛЬЗУЙТЕ UniversalBookingModal**
...
#### 🔧 **ЛОКАЛЬНЫЙ КАЛЬКУЛЯТОР (только для справки - НЕ для новых туров)**

**📌 ИСПОЛЬЗУЕТСЯ В ГОТОВЫХ ТУРАХ (PhiPhi, Dostoprimechatelnosti)** - НЕ ТРОГАТЬ!
...
### 🎯 **ПОШАГОВЫЙ ПЕРЕНОС ИЗ PhiPhi2Days1Night.tsx**

#### 📋 **ШАГ 1: КОПИРОВАНИЕ ФАЙЛА**
```bash
1. ✅ СКОПИРОВАТЬ src/pages/PhiPhi2Days1Night.tsx
2. ✅ ПЕРЕИМЕНОВАТЬ в YourNewTour.tsx  
3. ✅ ЗАМЕНИТЬ component name: export default function YourNewTour()
```

#### 📋 **ШАГ 2: ИМПОРТЫ И ИЗОБРАЖЕНИЯ**
```typescript
// ✅ ЗАМЕНИТЬ ВСЕ ИМПОРТЫ ИЗОБРАЖЕНИЙ:
...
#### 📋 **ШАГ 3: ДАННЫЕ ТУРА (excursion объект)**
```typescript
// ✅ ЗАМЕНИТЬ ВСЕ ПОЛЯ В excursion:
const excursion = {
  title: "Ваше название тура",           // ← ЗАМЕНИТЬ
  subtitle: "Ваш подзаголовок",          // ← ЗАМЕНИТЬ  
  priceAdult: 1900,                      // ← ЗАМЕНИТЬ на вашу цену
  priceChild: 1400,                      // ← ЗАМЕНИТЬ на вашу цену
  currency: "฿",                         // ← ОСТАВИТЬ ฿
  duration: "1 день (8 часов)",          // ← ЗАМЕНИТЬ
  groupSize: "до 30 человек",            // ← ЗАМЕНИТЬ
  rating: 4.8,                           // ← ЗАМЕНИТЬ
  reviewsCount: 243,                     // ← ЗАМЕНИТЬ
  mainImage: newMainImage,               // ← ЗАМЕНИТЬ на ваш импорт!
  // ... ЗАМЕНИТЬ ВСЕ ОСТАЛЬНЫЕ ПОЛЯ
};
```

#### 📋 **ШАГ 4: НАСТРОИТЬ UniversalBookingModal (РЕКОМЕНДУЕМЫЙ)**
```typescript
// ✅ ДЛЯ НОВЫХ ТУРОВ ИСПОЛЬЗУЙТЕ UniversalBookingModal:
...
```typescript
// Проверить useState и totalPrice:
const [adults, setAdults] = useState(1);
const [children, setChildren] = useState(0);
const totalPrice = adults * excursion.priceAdult + children * excursion.priceChild;

// Проверить кнопки +/-:
onClick={() => setAdults(Math.max(1, adults - 1))}  // мин. 1
onClick={() => setChildren(Math.max(0, children - 1))} // мин. 0
```

**❌ "Форма бронирования не открывается"**
```typescript
// Проверить handleBooking и showBookingForm:
...
🚨 ЦЕНТРАЛИЗАЦИЯ КАЛЬКУЛЯТОРА И ТЕГОВ:

✅ ОБЯЗАТЕЛЬНОЕ ИСПОЛЬЗОВАНИЕ UniversalBookingModal:
- ВСЕГДА использовать UniversalBookingModal для бронирования туров
- ЗАПРЕЩЕНО создавать локальные формы бронирования
- Передавать tourData в правильном формате:
  {
    id, title, priceAdult, priceChild, currency, duration, 
    groupSize, rating, reviewsCount, route
  }

✅ DATA-DRIVEN ТЕГИ:
- Теги ТОЛЬКО из данных туров (excursion.tags)
- ЗАПРЕЩЕНО hardcoded теги в компонентах
- Теги должны быть кликабельными для фильтрации
- ОБЯЗАТЕЛЬНО использовать компонент TourTags из @/components/TourTags

✅ WordPress-ПРИНЦИП:
- Изменения в одном месте влияют на все
- UniversalBookingModal - единый источник калькулятора
- Централизованные данные туров
- TourTags - единый компонент для отображения тегов
