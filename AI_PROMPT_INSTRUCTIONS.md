...
### 🔧 **АЛЬТЕРНАТИВНЫЙ КАЛЬКУЛЯТОР: UniversalBookingModal**

**📌 КОГДА ИСПОЛЬЗОВАТЬ:** Если нужен простой модал без сложного калькулятора

#### ✅ **ПРИМЕР ИСПОЛЬЗОВАНИЯ (как в PearlsAndamanSea.tsx):**
```typescript
// 1️⃣ ИМПОРТ:
...
#### 📋 **ШАГ 4: НЕ ТРОГАТЬ КАЛЬКУЛЯТОР!**
```typescript
// ❌ НЕ МЕНЯТЬ ЭТИ СТРОКИ - ОНИ РАБОТАЮТ:
const [adults, setAdults] = useState(1);
const [children, setChildren] = useState(0);
const totalPrice = adults * excursion.priceAdult + children * excursion.priceChild;

// ❌ НЕ МЕНЯТЬ КНОПКИ +/- - ОНИ РАБОТАЮТ:
onClick={() => setAdults(Math.max(1, adults - 1))}
onClick={() => setAdults(adults + 1)}
onClick={() => setChildren(Math.max(0, children - 1))}
onClick={() => setChildren(children + 1)}

// ❌ НЕ МЕНЯТЬ handleBooking - ОНА РАБОТАЕТ:
const handleBooking = useCallback(() => { ... }, [adults, children, totalPrice, excursion.title]);
```
