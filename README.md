...
## 🛠️ Админка заявок (`#/admin`)

- URL: https://phukeo.com/#/admin
- Источник данных: локальное хранилище браузера `localStorage` (ключ `bookingOrders`).
- Как заполнить:
	1) Откройте любой тур (или карточку на главной) и нажмите «Забронировать тур».
	2) Заполните минимум поля: Имя, Телефон, Дата и отправьте.
	3) Заявка сохранится в `localStorage` и отправится в Telegram; админка отобразит её.
- Пусто? Значит ещё не было бронирований в этом браузере/устройстве. Создайте тестовую заявку, как описано выше.

Примечания:
- Кнопка «Забронировать тур» в карточках защищена от перехвата клика Link-обёрткой (capture-события, `preventDefault/stopPropagation`).
- Для проверки продакшн-сборки смотрите маркер версии: https://phukeo.com/__build.txt
