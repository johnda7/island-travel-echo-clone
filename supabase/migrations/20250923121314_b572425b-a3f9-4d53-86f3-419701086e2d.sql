-- Обновляем существующий тур "Острова Рача и Корал" с правильными данными из оригинального сайта
UPDATE public.tours SET
  title = 'Острова Рача и Корал',
  subtitle = 'Экскурсия на острова Рача за 1 день с обедом',
  description = 'Рача — Коралловые острова с пирса Раваи — 1 день
...
16:00 Прибытие на пирс Раваи и трансфер обратно в отель.',
  short_description = 'Посетите два красивейших острова за один день: остров Рача с белоснежными пляжами и остров Корал с водными развлечениями.',
  price_adult = 1300,
  price_child = 1200,
  currency = 'THB',
  duration = '8 часов (08:30-16:00)',
  ...
  tags = ARRAY['Острова', 'Сноркелинг', 'Пляжный отдых', 'Однодневный тур', 'Рача', 'Корал'],
  category_id = (SELECT id FROM public.tour_categories WHERE slug = 'island-tours'),
  is_featured = true,
  is_active = true,
  sort_order = 3,
  meta_title = 'Острова Рача и Корал - экскурсия на 1 день с обедом | PhuketGo',
  meta_description = 'Посетите острова Рача и Корал за один день. Сноркелинг, пляжный отдых, обед на острове. Цена от 1300 THB. Трансфер включен.',
  updated_at = now()
WHERE slug = 'racha-coral-islands-speedboat';

-- Сначала удаляем старые данные из itinerary и gallery для этого тура
DELETE FROM public.tour_itinerary WHERE tour_id = (SELECT id FROM public.tours WHERE slug = 'racha-coral-islands-speedboat');
DELETE FROM public.tour_gallery WHERE tour_id = (SELECT id FROM public.tours WHERE slug = 'racha-coral-islands-speedboat');
